...
- **`mcp-server/receipts_status_server.py`** — the server. One tool: `get_receipts_status()`, no arguments for v1. **Correction, 2026-09-24:** originally named `receipts_status()`; renamed after live testing showed JARVIS's bridge (`bridge/server.mjs`'s `decideTool()`) auto-approves read-only tools by matching a recognized verb prefix (`get`/`list`/`read`/`search`/etc.) in the tool name — a noun-only name silently falls through to requiring `ALLOW_WRITES` and gets denied even though the tool is genuinely read-only. Any future tool added to this or other JARVIS-facing MCP servers needs a `READ_VERB`-matching name for the same reason.
