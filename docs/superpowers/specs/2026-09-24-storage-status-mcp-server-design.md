...
**Status:** DRAFT r4 — three fresh-eyes reviews (2026-09-24: 19 + 13 + 12 findings) ruled and folded in; targeted re-read of the r4 edits pending, then Gardiner's approval
**Date:** 2026-09-24
...
- **ops** — built from `active_ops` (heartbeats), `ops_log` (full log, the pool) and `drives`. Mirrors `ops_sweep` (`summarize.py:~786-900`) exactly, without any of its writes:
  ...
  4. **`abandoned` (sweep step 6):** (i) pool entries with `state == "running"`, excluding the synthetic unreadable-heartbeat entry (step 7's territory) and legacy `checkin_scan` entries (`op_id` starting `legacy_` — the sweep never closes legacy heartbeats), where `last_beat` is missing/unparseable, **or** older than `summarize.ABANDON_S` (24 h), **or** `summarize._find_newer_match(e, pool, e["op_id"])` returns a newer same-family run; plus (ii) the unresolved (`not is_resolved`) sweep auto-closures excluded from rule 3. → `warn`.
...
- Reach (merged into one sentence): three groups, each rendered once, joined with ", and ":
  ...
  Example: "Projects-Syno isn't mounted, and I couldn't read live operations and the operations log." When there are **no** problem clauses but reach is non-empty, prefix "Everything I could read looks fine, but " (lower-casing the first group).
