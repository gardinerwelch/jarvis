...
**Status:** DRAFT r3 — two fresh-eyes reviews (2026-09-24: 19 + 13 findings) ruled and folded in; targeted third pass on Ops + Headline pending, then Gardiner's approval
**Date:** 2026-09-24
...
- `pct = ceil(100 * used / (used + avail))` — df's own formula (live check: Projects-Syno → 90, matching `df`; the naive `1 − bavail/blocks` gives 89).
A volume whose thread misses the deadline is recorded `{name, timed_out: true}`.

**Timeouts:** start all threads (`threading.Thread(daemon=True)`), set `deadline = time.monotonic() + 5`, then `t.join(max(0, deadline - time.monotonic()))` for each — one shared 5 s budget, not 5 s per thread. Each thread writes only into its **own private holder** object; after the joins, `collect()` copies each holder's value once and never hands a holder out, so a late-finishing abandoned thread can't mutate the returned snapshot mid-serialisation. A thread still alive at the deadline is abandoned (daemon; no locks or shared mutable state — verified) and its source is `ok: false, error: "timed out"`. No `ThreadPoolExecutor` (its shutdown waits on hung workers). A client cancel lets the ≤5 s worker finish (`to_thread.run_sync` default `cancellable=False`) — acceptable. No upstream change to file-management's existing modules.
...
```
{
  ...
  "sections": {
    "capacity": { status, as_of, source: "live",   reason?, items: [{name, used_bytes, size_bytes, pct, level} | {name, timed_out: true}], missing: [name] },
...
- **capacity** — `level`: `critical` ≥ 95, `warn` ≥ 85, else `ok`. A timed-out volume → `warn`. `EXPECTED_MOUNTS = {"Projects-Syno": "Projects-Syno", "_Inbox": "the Inbox share"}` (mount name → spoken name; names verified against `/Volumes`): any not mounted → listed in `missing`, section at least `warn`. Section status = worst of items and missing.
...
- **ops** — built from `active_ops` (heartbeats), `ops_log` (full log, the pool) and `drives`. Mirrors `ops_sweep` (`summarize.py:~786-900`) exactly, without any of its writes:
  1. **Dedupe by `op_id` (sweep step 3):** if an `op_id` exists in both heartbeats and the log: when the log entry has `terminal_source == "finish"`, drop the heartbeat copy (sweep step 3 treats it as dead weight); otherwise keep the **log** copy (it carries `resolved_by`/`resolution_note`). Result: one entry per `op_id` = the **pool**.
  2. **Field notes:** heartbeat entries (from `_finalize_new_style_entry`) carry `started_at`/`finished_at`/`op_type`/`subject`/`target` and `resolved_by: None` but never `logged_at`; log entries carry `logged_at` and may lack `resolved_by`/`resolution_note` keys — always `.get()`. The "time" of an entry is `finished_at or logged_at or ""`; running and legacy entries therefore have time `""` and can never supersede anything (correct — matches the sweep).
  3. **`failed` (sweep step 5):** pool entries with `state == "failed"`, dropped if `summarize.is_resolved(e)` (honours `resolved_by` **and** `resolution_note` — deliberately broader than step 5's `resolved_by`-only check, per the 2026-08-09 ruling in `is_resolved`'s docstring) or if any *other* pool entry satisfies `summarize._same_run_family(e, other)` and has a strictly later time (any terminal state supersedes, as the sweep does; a newer failure supersedes an older one). The sweep also resolves `paused`, but paused is a first-class terminal state, not a failure, so it is never reported. What remains → `critical`.
  4. **`abandoned` (sweep step 6):** pool entries with `state == "running"`, excluding the synthetic unreadable-heartbeat entry (step 7's territory), where `last_beat` is missing/unparseable, **or** older than `summarize.ABANDON_S` (24 h), **or** `summarize._find_newer_match(e, pool, e["op_id"])` returns a newer same-family run. → `warn`.
  5. **`stalled`:** `state == "running"`, `stale: True`, not abandoned — including the synthetic unreadable-heartbeat entry, identified by `op_type is None and last_beat is None` (label "unreadable heartbeat — inspect"; the repo's designed visible alarm). → `warn`.
...
**Algorithm:**
1. Build two lists. **Problem clauses**, in priority order: critical capacity → failed operation → abandoned operation → stalled operation → drive health warning → overdue/abnormal backup → warn-level capacity → open lanes. **Reach clauses**: missing expected mounts, timed-out volumes, and failed sub-sources/sections.
...
- Critical / warn capacity: "Projects-Syno is at 96 percent." (two or more → "Projects-Syno and LaCie are over 85 percent.")
...
- Drive warning: "The LaCie has a health warning." (count if > 1: "Two drives have health warnings.")
